...
func TestFindAccountMarketTrades(t *testing.T) {
	...
	_, trades := TradeDaoSqlite.FindAllTrades("WETH-DAI")
	assert.EqualValues(t, 0, len(trades))

	trade := NewTrade("WETH-DAI", true)
	_ = TradeDaoSqlite.InsertTrade(trade)

	var count int64
	count, trades = TradeDaoSqlite.FindAccountMarketTrades(trade.Maker, "WETH-DAI", common.STATUS_SUCCESSFUL, 1, 0)
	assert.EqualValues(t, 1, len(trades))
	assert.EqualValues(t, 1, count)

	count, trades = TradeDaoSqlite.FindAccountMarketTrades(trade.Maker, "WETH-DAI", common.STATUS_SUCCESSFUL, 2, 0)
	assert.EqualValues(t, 1, len(trades))
	assert.EqualValues(t, 1, count)

	//count, trades = TradeDaoSqlite.FindAccountMarketTrades(trade.Maker, "WETH-DAI", common.STATUS_FAILED, 2, 0)
	//assert.EqualValues(t, 0, len(trades))
...
func TestTradeDao_FindAllTrades(t *testing.T) {
	...
	_ = TradeDaoSqlite.InsertTrade(trade1)
	_ = TradeDaoSqlite.InsertTrade(trade2)
	_ = TradeDaoSqlite.InsertTrade(trade3)
...
func TestTradeDao_InsertAndFindOneAndUpdateTrade(t *testing.T) {
	...
	trade := NewTrade("WETH-DAI", true)
	err := TradeDaoSqlite.InsertTrade(trade)
	assert.Nil(t, err)

	dbTrades := TradeDaoSqlite.FindTradesByHash(trade.TransactionHash)
...
func TestTradeDao_UpdateTradeAndUpdateByStatus(t *testing.T) {
	...
	dbTrade := TradeDaoSqlite.FindTradeByID(trade.ID)
	...
	_ = TradeDaoSqlite.UpdateTrade(dbTrade)
	dbTrade2 := TradeDaoSqlite.FindTradeByID(trade.ID)
...
func TestTradeDao_FindTradesByMarket(t *testing.T) {
	...
	_, trades1 := TradeDaoSqlite.FindAllTrades("WETH-DAI")
	assert.EqualValues(t, 3, len(trades1))

	trades2 := TradeDaoSqlite.FindTradesByMarket("WETH-DAI", time4, time5)
	assert.EqualValues(t, 3, len(trades2))

	trades3 := TradeDaoSqlite.FindTradesByMarket("WETH-DAI", time2, time5)
...
func Test_PG_FindAccountMarketTrades(t *testing.T) {
	...
	//count, trades = TradeDaoPG.FindAccountMarketTrades(trade.Maker, "WETH-DAI", common.STATUS_FAILED, 2, 0)
	//assert.EqualValues(t, 0, len(trades))
	//assert.EqualValues(t, 0, count)
}
